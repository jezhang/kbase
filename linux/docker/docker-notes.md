...
docker run --name nginx -p 80:80 -p 443:443 -v /root/nginx/conf.d:/etc/nginx/conf.d -v /root/nginx/tcp.d:/etc/nginx/tcp.d -v /etc/letsencrypt:/etc/letsencrypt -v /root/nginx/nginx.conf:/etc/nginx/nginx.conf:rw -v /root/downloads:/root -d nginx
...
docker run --name ubuntu -v /root/downloads:/root -t -i ubuntu:16.04 /bin/bash
